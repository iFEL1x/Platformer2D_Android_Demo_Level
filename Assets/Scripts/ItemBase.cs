...
public class ItemBase : ScriptableObject
{
    ...
    public void CreateItem() //Создаем лист с предмет в нем
    {
        if (items == null)
        {
            items = new List<Item>();
        ...
        items.Add(item);
        currentItem = item;
        currentIndex = items.Count - 1; //Устанавливаем Index где он равен колличетву пердметов в "List<Item> items" минус еденица (-1)
    }

    public void RemoveItem() //Удаляем предметы из листа
    {
        if (items == null)
        {
            return;
